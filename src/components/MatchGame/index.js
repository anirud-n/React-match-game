...
class MatchGame extends Component {
  ...
  compareTumbAndMain = id => {
    const {mainObj, score} = this.state
    if (mainObj.id === id) {
      const randObjIndex = Math.floor(Math.random() * imagesList.length)
      this.setState({
        score: score + 1,
        mainObj: imagesList[randObjIndex],
      })
    } else {
  ...
  render() {
    ...
    const selectedTabImageList = updatedImagesList.filter(
      eachObj => eachObj.category === selectedTabId,
    )
    ...
    if (gameStatus === 'gameOn') {
      displayItem = (
        <div className="game-cont">
          <img src={mainObj.imageUrl} className="main-img" alt="match" />
          <ul className="tab-list-cont">
            {updatedTabsList.map(eachObj => (
              <TabItem
                eachObj={eachObj}
                key={eachObj.tabId}
          ...
          <ul className="thumbnail-list-cont">
            {selectedTabImageList.map(eachObj => (
              <ImageButtons
                eachObj={eachObj}
                key={eachObj.id}
                compareTumbAndMain={this.compareTumbAndMain}
              />
            ))}
          </ul>
        </div>
          ...
          <div className="game-over-card">
            ...
            >
              <img
                ...
                className="reset-img"
              />
              <p className="btn-txt">PLAY AGAIN</p>
            </button>
          </div>
        </div>
      )
      ...
      <div className="bg-cont">
        <ul className="nav-cont">
          ...
          <li className="score-cont">
            <p className="score">
              Score: <span className="yellow-text">{score}</span>
            </p>
            <img
              ...
              className="clock-img"
            />
            <p className="yellow-text">{time} sec</p>
          </li>
        </ul>
        {displayItem}
      </div>
